...
const Modal = ({title, body, btn,  setOpen, btnClass, closeBtnClass, btnGroupClass, ...rest}: main) => {
    ...
    return <SimpleModal setOpen={setOpen} {...rest}>
        ...
        <Flex gap={2} className={tJS.functions.mergeClassNames(style.btnsGroup, btnGroupClass)} justifyContent="end">
            <button onClick={closeModal} className={tJS.functions.mergeClassNames(style.closeBtn, style.btn, closeBtnClass)}>CLOSE</button>
            {label && <button onClick={onClick} className={tJS.functions.mergeClassNames(className, style.btn)}>{label}</button>}
        </Flex>
    </SimpleModal>
}
